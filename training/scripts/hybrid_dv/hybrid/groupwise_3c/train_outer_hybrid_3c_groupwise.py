...
"""
Hybrid baseline on OUTER (3-class genotype: 0/1/2), with groupwise heads.
...
def main():
    ...
    ap.add_argument("--save_dir", type=str, default="training/out/models/baselines/hybrid_groupwise_mlp")
    ...
    if args.arch == "linear":
        print(f"[Model] arch=groupwise-linear in_dim={in_dim} n_classes=3 add_masks={add_masks}")
    else:
        print(f"[Model] arch=groupwise-mlp in_dim={in_dim} n_classes=3 hidden={args.hidden} dropout={args.dropout} add_masks={add_masks}")

...
"""
Hybrid baseline on OUTER (3-class genotype: 0/1/2)
...
Example
-------
python training/scripts/hybrid_dv/train_outer_hybrid_3c.py \
  ...
  --save_dir training/out/models/baselines/hybrid_linear \
  --arch linear \
  --epochs 10
"""
...
def main():
    ...
    ap.add_argument("--save_dir", type=str, default="training/out/models/baselines/hybrid_mlp")
    ap.add_argument("--out_csv", type=str, default=None)

    ap.add_argument("--save_best_on", type=str, default="val_macro_f1",
                    choices=["val_macro_f1", "val_acc", "val_weighted_f1", "val_variant_recall"])
    ...
    if args.arch == "linear":
        print(f"[Model] arch=linear in_dim={in_dim} n_classes=3 add_masks={add_masks} (hidden/dropout ignored)")
    else:
        print(f"[Model] arch=mlp in_dim={in_dim} n_classes=3 hidden={args.hidden} dropout={args.dropout} add_masks={add_masks}")
    ...
    def score_from_val(val_m: Dict[str, Any]) -> float:
        ...
        if args.save_best_on == "val_variant_recall":
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        val_m = eval_sharded_3c(model, val_by_shard, device, args.batch_size, add_masks)
        test_m = eval_sharded_3c(model, test_by_shard, device, args.batch_size, add_masks)

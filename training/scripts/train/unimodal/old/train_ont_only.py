...
"""
ONT-only baseline training on OUTER (3-class genotype: 0/1/2)

Policy
------
- Visible groups: group01 + group11
- Target:
    * group01 -> label_ont
...
Example
-------
python training/scripts/hybrid_dv/train_outer_ont_only.py \
  ...
  --save_dir training/out/models/baselines/ont_only_linear \
  --arch linear \
  --epochs 10
"""
    ...
    for sp in list_shards(outer_dir):
        ...
        for i, locus in enumerate(loci_list):
            ...
            if group == 1:
                ...
                if label_mismatch == 1:
                    continue
                y3 = label_ont  # same as label_ill under consensus
            else:
                continue
...
def eval_sharded_3c(model, by_shard: Dict[str, List[Ref]], device, batch_size: int) -> Dict[str, Any]:
    ...
    for shard, items in by_shard.items():
        ...
        for i in range(0, len(rows), batch_size):
            ...
            x = torch.from_numpy(ont_e[rr]).to(device)
            logits = model(x)
...
def main():
    ...
    ap.add_argument("--save_dir", type=str, default="training/out/models/baselines/ont_only_mlp")
    ...
    if not refs_all:
        raise SystemExit("No refs indexed after applying ONT-only consensus policy.")
    ...
    print(f"[Split/FILTER] examples (ONT-only consensus: groups 01+11, drop group11 label_mismatch): "

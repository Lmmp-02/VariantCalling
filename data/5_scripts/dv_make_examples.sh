...
Optional but strongly recommended:
  ...
  --region REGION            (alias of --regions; e.g. chr20)
  --regions REGIONS          (optional; e.g. chr20 or chr20,chr21)
  --regions_id ID            (naming id; default: sanitized from --regions)
                            (required if --regions is omitted and dataset_id is not given)
  --coverage_tag TAG         (e.g. harmonized_40x; default: omitted from dataset_id)
  --dataset_id ID            (override computed dataset_id)
  --mode training|calling    (default: training)

Inputs:
  --ref PATH                 (default: data/2_reference/.../GRCh38_no_alt_plus_hs38d1.<regions_id>.fa)
  --truth_vcf PATH           (required in training mode; default: data/3_truth/<sample>/<sample>_<regions_id>.truth.vcf.gz)
  --confident_bed PATH       (required in training mode; default: data/3_truth/<sample>/<sample>_<regions_id>.confident.bed)
...
obj = {
    ...
    "regions": os.environ.get("REGIONS") or None,
    "regions_id": os.environ["REGIONS_ID"],
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --regions_id) REGIONS_ID="$2"; shift 2 ;;
...
if [[ -z "$REGIONS_ID" ]]; then
  if [[ -n "$REGIONS" ]]; then
    REGIONS_ID="$(sanitize_regions_id "$REGIONS")"
  elif [[ -n "$DATASET_ID" ]]; then
    REGIONS_ID="unspecified"
  else
    die "Missing --regions_id (required when --regions is omitted and --dataset_id is not provided)."
  fi
fi

if [[ -z "$DATASET_ID" ]]; then
  DATASET_ID="$(echo "$SAMPLE" | tr '[:upper:]' '[:lower:]')_${REGIONS_ID}"
  if [[ -n "$COVERAGE_TAG" ]]; then
    DATASET_ID+="_${COVERAGE_TAG}"
  fi
fi

# ---- optional region filters passed to DeepVariant ----
...
if [[ -n "$REGIONS" ]]; then
  ...
  for _r in "${_raw_regions[@]}"; do
    _r="$(echo "$_r" | xargs)"
    [[ -n "$_r" ]] || continue
    REGION_ARGS+=( --regions "$_r" )
  done
  [[ ${#REGION_ARGS[@]} -gt 0 ]] || die "No valid regions parsed from --regions: $REGIONS"
fi

if [[ -z "$REF" ]]; then
  REF="data/2_reference/GRCh38_no_alt_plus_hs38d1/GRCh38_no_alt_plus_hs38d1.${REGIONS_ID}.fa"
fi
REF="$(to_abs "$REF")"
...
if [[ "$MODE" == "training" ]]; then
  if [[ -z "$TRUTH_VCF" ]]; then
    TRUTH_VCF="data/3_truth/${SAMPLE}/${SAMPLE}_${REGIONS_ID}.truth.vcf.gz"
  fi
  if [[ -z "$CONF_BED" ]]; then
    CONF_BED="data/3_truth/${SAMPLE}/${SAMPLE}_${REGIONS_ID}.confident.bed"
  fi
...
export DATASET_ID SAMPLE REGIONS REGIONS_ID COVERAGE_TAG TECH DV_PRESET MODE

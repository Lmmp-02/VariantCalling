...
# Regional BAM retrieval for GIAB samples used in VariantCalling.
#
# Illumina:
#   - Remote regional extraction from GIAB/NIST BAM via samtools
#
# ONT:
#   - Remote regional extraction from ONT Open Data BAMs via samtools
#   - Optional merge of multiple regional BAMs after extraction
#
...
#   - Region names are resolved dynamically from the remote BAM header/aliases.
#   - Existing valid BAM+BAl pairs are reused unless --overwrite is specified.
#
...
Optional:
  ...
  --overwrite                   Force re-download / re-generate existing outputs
...
is_valid_bam_with_index() {
  local bam="$1"
  local bai="${bam}.bai"

  [[ -f "$bam" ]] || return 1
  [[ -f "$bai" ]] || return 1
  samtools quickcheck "$bam" || return 1
  return 0
}

should_skip_bam_output() {
  local bam="$1"
  ...
  if is_valid_bam_with_index "$bam"; then
    return 0
  fi
...
if [[ "$TECH" == "Illumina" ]]; then
  ...
  if should_skip_bam_output "$OUT_BAM"; then
    log_info "Reusing existing valid regional BAM: ${OUT_BAM}"
  else
    log_step "Resolving Illumina region names from remote BAM header..."
    ILLUMINA_REGIONS_STR=$(resolve_regions_from_remote_bam "$BAM_SPEC" "${REGIONS[@]}")
    [[ -n "$ILLUMINA_REGIONS_STR" ]] || { log_error "resolved Illumina region list is empty"; exit 1; }
    mapfile -t ILLUMINA_REMOTE_REGIONS <<< "$ILLUMINA_REGIONS_STR"

    log_info "Resolved remote regions: ${ILLUMINA_REMOTE_REGIONS[*]}"

    if [[ "$OVERWRITE" == true ]]; then
      rm_if_exists "$OUT_BAM"
      rm_if_exists "$OUT_BAI"
    fi

    run_remote_extract_sort "$BAM_SPEC" "$OUT_BAM" "${ILLUMINA_REMOTE_REGIONS[@]}"
  ...
  write_metadata_json \
    ...
    "" \
    "" \
    "Illumina BAM used as remote indexed source. Requested regions were resolved dynamically from the BAM header/aliases before extraction. Output naming follows source label semantics (2x250 for HG002-HG004, 300x for HG005). Extraction uses retries and promotes only BAMs that pass quickcheck."
...
if [[ "$TECH" == "ONT" ]]; then
  ...
  if [[ ${#PER_RUN_BAMS[@]} -gt 1 && "$MERGE_ONT" == true ]]; then
    ...
    if should_skip_bam_output "$MERGED"; then
      log_info "Reusing existing merged BAM: ${MERGED}"
    else
      if [[ "$OVERWRITE" == true ]]; then
        rm_if_exists "$MERGED"
        rm_if_exists "$MERGED_BAI"
      fi
